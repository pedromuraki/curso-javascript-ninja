(function() {
	...
	*/
	...
	function hasClass(markup, cssClass) {
		var markupArr = markup.match(/['"].+?['"]/gm);
		var cssClassRegex = new RegExp(cssClass, 'g');
		
		//return markupArr.join('\s').replace(/["']/g, '').match(cssClassRegex) !== null ? true : false;
		return cssClassRegex.test( markupArr.join('\s').replace(/["']/g, '') );
	}
