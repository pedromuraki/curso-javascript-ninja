(function() {
	...
	*/
	...
	function formatCPF(cpf) {
		...
		// var newCPF = cpf.replace(regex, function(reg, g1, g2, g3, g4) {
		// 	return g1 + '.' + g2 + '.' + g3 + '-' + g4;
		// });
		var newCPF = cpf.replace(regex, '$1.$2.$3-$4');
		return newCPF;
	}
	...
	*/
	...
	var htmlTags = '<div><ul><li></li><li></li><li><span></span></li></ul></div>';
	console.log( htmlTags.match( /<\w+><\/\w+>/g ) );
	...
	*/
	...
	var htmlText = '<h1>Título da página</h1><p>Este é um parágrafo</p><footer>Rodapé</footer>';
	var htmlTextArray = htmlText.match( /(<\w+>)([^<]+)(<\/\w+>)/g );
	htmlTextArray.forEach( function(item){
		console.log( item.replace( /(<\w+>)([^<]+)(<\/\w+>)/g, function(reg, op, ctn, cl) {
			return op + 'O texto dentro da tag ' + '"' + op.replace( /[<>]/g, '' ) + '"' + ' é ' + '"' + ctn + '".' + cl;
		} ) );
	});
