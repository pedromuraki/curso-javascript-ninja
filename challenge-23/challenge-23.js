(function(){
	...
	function addOperator() {
		if ( $result.value.match(/[+\-*\/]$/g) )
			$result.value = $result.value.replace(/[+\-*\/]$/g, this.value);
		else
			$result.value += this.value;
	}
	...
	function doOperation() {
		...
		// console.log($result.value.match(/\d+|[+\-*\/]/g))
		
		var operationArray = $result.value.match(/\d+|[+\-*\/]/g).map(function(item){
			if ( item.match(/\d+/g) )
				return Number(item);
			return item;
		});
		...
		// var resultado = operationArray.reduce(function(prev, cur){
		// 	return prev + cur;
		// }, 0);
	...
	$operator.forEach(function(item){
		item.addEventListener('click', addOperator, false);
	});

	$btnResult.addEventListener('click', doOperation, false);
